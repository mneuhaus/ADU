 ...
 */
class DbCommandController extends \TYPO3\Flow\Cli\CommandController {
	 ...
	 *
	 * @param boolean $confirm don't ask for confirmation
	 * @return void
	 */
	public function purgeAllCommand($confirm = FALSE) {
        if ($confirm !== TRUE) {
            echo 'you need to --confirm to execute this command!';
            return;
        }
     ...
     * @return void
     */
    public function dropAllCommand($confirm = FALSE) {
        if ($confirm !== TRUE) {
            echo 'you need to --confirm to execute this command!';
            return;
        }
     ...
     * @return void
     */
    public function restoreCommand($snapshot) {
        $connection = $this->entityManager->getConnection();
        $host = $connection->getHost();
        $database = $connection->getDatabase();
        $username = $connection->getUsername();
        ...
        system($command);
        $this->outputLine('restored snapshot "' . $snapshot . '"');
    }
}
