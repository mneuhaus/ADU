...
class Parser implements \TYPO3\TypoScript\Core\ParserInterface {
	...
	protected function parseObjectDefinition($typoScriptLine) {
		...
		switch ($matches['Operator']) {
			...
			case '<<' :
				...
				break;
			case '>>' :
				$this->parseValueUnProcessing($matches['Value'], $objectPath);
				break;
		}
	 ...
	 * @return void
	 */
	protected function parseValueUnProcessing($indexAndMethodCall, $objectPropertyPath) {
		$objectPropertyPathArray = $this->getParsedObjectPath($objectPropertyPath);
		...
		if (isset($typoScriptObject['__processors'][$propertyName][$indexAndMethodCall])) {
			unset($typoScriptObject['__processors'][$propertyName][$indexAndMethodCall]);
		} else {
			unset($typoScriptObject['__processors'][$propertyName]);
		}
	}
