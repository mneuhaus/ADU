...
class ActionController extends AbstractController {
	 ...
	 * @throws \TYPO3\Flow\Mvc\Exception\NoSuchViewException if no view can be resolved
	 */
	protected function resolveView() {
		$viewsConfiguration = $this->getViewsConfiguration();

		if (isset($viewsConfiguration['viewClassName'])) {
			$viewClassName = $viewsConfiguration['viewClassName'];
		} elseif ($this->resolveViewObjectName() !== FALSE) {
			$viewClassName = $this->resolveViewObjectName();
		} elseif ($this->defaultViewObjectName != '') {
			$viewClassName = $this->defaultViewObjectName;
		}

		$view = $this->objectManager->get($viewClassName);

		if (!isset($view)) {
			throw new \TYPO3\Flow\Mvc\Exception\NoSuchViewException('An view with the className "' . $viewClassName . '" does not exist.', 1352763182);
		}

		if (isset($viewsConfiguration['options'])) {
			$view->setOptions($viewsConfiguration['options']);
		}
	...
	/**
	 * This method merges the configurations from the package, controller
	 * and action levels into a single configuration array.
	 * Possible options are currently the viewClassName to specify
	 ...
	 */
	protected function getViewsConfiguration() {
		$path = $this->request->getControllerPackageKey();
		if ($this->request->getControllerSubpackageKey() !== NULL) {
			$path .= '.' . $this->request->getControllerSubpackageKey();
		}
		$configuration = $this->configurationManager->getConfiguration('Views', $path);

		$controllerConfiguration = $this->configurationManager->getConfiguration('Views', $path . '.controllers.' . $this->request->getControllerObjectName());
		if ($controllerConfiguration !== NULL) {
			$configuration = \TYPO3\Flow\Utility\Arrays::arrayMergeRecursiveOverrule($configuration, $controllerConfiguration);
		}
