...
class FlowAnnotationDriver implements \Doctrine\Common\Persistence\Mapping\Driver\MappingDriver, \TYPO3\Flow\Aop\Pointcut\PointcutFilterInterface {
	...
	public function loadMetadataForClass($className, \Doctrine\Common\Persistence\Mapping\ClassMetadata $metadata) {
		$cache = $this->cacheManager->getCache('Flow_Reflection_AnnotationDriver');
		$identifier = sha1($className);

		if (!$cache->has($identifier)) {
			$this->doLoadMetadataForClass($className, $metadata);
			$cache->set($identifier, get_object_vars($metadata));
		} else {
			foreach ($cache->get($identifier) as $key => $value) {
				$metadata->$key = $value;
			}
		}
	}
	 ...
	 * @todo adjust when Doctrine 2 supports value objects, see http://www.doctrine-project.org/jira/browse/DDC-93
	 */
	public function doLoadMetadataForClass($className, \Doctrine\Common\Persistence\Mapping\ClassMetadata $metadata) {
